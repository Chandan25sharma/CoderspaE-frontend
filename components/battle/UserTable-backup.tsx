...
const UserTable: React.FC<UserTableProps> = ({ battleMode }) => {
  ...
  const fetchProblems = useCallback(async () => {
    try {
      const response = await fetch(`/api/problems?battleMode=${battleMode}&limit=10`);
      if (!response.ok) {
            ...
            <div className="p-6">
              ...
              <div style={{ flex: 1, minWidth: 250, position: 'relative', marginBottom: 20 }}>
                <label style={{ fontWeight: 'bold', marginBottom: 6, display: 'block', color: '#333' }}>
                  Select Problem (Quick Battle Mode Only)
                </label>
                  ...
                  {showDropdown && (
                    <div style={{
                      ...
                      {filteredProblems.length > 0 ? (
                        filteredProblems.map((problem) => (
                          ...
                          >
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <div>
                                <p style={{ fontWeight: 'medium', color: '#333', margin: 0, fontSize: 14 }}>
                                  {problem.title}
                                </p>
                        ...
                        <div style={{ padding: 10, textAlign: 'center', color: '#666', fontSize: 14 }}>
                          {searchTerm ? `No Quick Battle problems found for "${searchTerm}"` : `Loading Quick Battle problems...`}
                        </div>
                      )}
                    </div>
                  )}
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button
                  onClick={() => {
                    if (selectedProblem) {
                      alert(`🚀 Challenge sent to ${selectedUser.username} with problem: ${selectedProblem.title}`);
                      closeChallengeModal();
                    } else {
