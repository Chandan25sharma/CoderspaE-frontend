...
const NarrativeModePage = () => {
    ...
    const initialComments: Comment[] = [
      {
        ...
        message: 'Love the adventure-themed problems! 📚⚔️',
        timestamp: '1 min ago'
      },
      {
        id: '2',
        username: 'QuestMaster',
        message: 'The narrative makes coding so much more engaging',
        timestamp: '4 min ago'
      },
      {
        id: '3',
        username: 'EpicCoder',
        message: 'Just completed the Dragon\'s Algorithm quest! 🐉',
        timestamp: '7 min ago'
      }
    ];
  ...
  const fetchLiveBattles = async (refresh = false) => {
    try {
      ...
      const response = await fetch('/api/battles?battleMode=narrative-mode');
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-center h-96">
            <div className="text-center">
              ...
              <p className="text-gray-300">Loading Narrative Mode adventures...</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        >
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-white">Narrative Mode</h1>
          </div>
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">
            Embark on coding adventures through story-driven programming challenges
          </p>
        </motion.div>
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            >
              <div className="flex gap-2">
                {[
                  { id: 'battles', label: 'Live Quests', icon: MonitorPlay },
                  { id: 'users', label: 'Adventurers', icon: Users },
                  { id: 'problems', label: 'Story Problems', icon: Scroll }
                ].map((tab) => (
                  <button
                    key={tab.id}
                ...
                <div className="bg-gray-900/50 backdrop-blur-xl rounded-2xl border border-gray-800/50 p-6">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-2 px-3 py-1 bg-orange-600/20 rounded-full">
                        ...
                        <span className="text-orange-400 text-sm font-medium">LIVE QUESTS</span>
                      </div>
                      <h2 className="text-2xl font-bold text-white">Active Adventures</h2>
                    </div>
                    <button
                      onClick={handleRefresh}
                      disabled={refreshing}
                      className="flex items-center gap-2 px-4 py-2 bg-orange-600/20 hover:bg-orange-600/30 border border-orange-500/30 rounded-lg text-orange-400 text-sm transition-colors disabled:opacity-50"
                    >
                      {refreshing ? (
                  ...
                  {error ? (
                    <div className="text-center text-red-400 py-12">
                      <p className="text-lg font-semibold mb-2">Error Loading Adventures</p>
                    ...
                    <div className="text-center text-gray-400 py-12">
                      ...
                      <p className="text-lg font-semibold mb-2">No Active Adventures</p>
                      <p className="text-sm">Start a new quest or check back later!</p>
                    </div>
                  ) : (
                    <div className="grid gap-4">
                      {battles.map((battle, index) => (
                        <motion.div
                          key={battle.id}
                          className="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6 hover:border-orange-500/30 transition-all"
                          initial={{ opacity: 0, y: 20 }}
                          animate={{ opacity: 1, y: 0 }}
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-4 text-sm text-gray-400">
                                  <span className="flex items-center gap-1">
                                    ...
                                    {battle.participants.length} adventurers
                                  </span>
                                  <span className="flex items-center gap-1">
                            ...
                            <div className="flex items-center gap-3">
                              <div className="flex gap-2">
                                <button
                                  ...
                                  className="flex items-center gap-2 px-4 py-2 bg-orange-600/20 hover:bg-orange-600/30 border border-orange-500/30 rounded-lg text-orange-400 text-sm transition-colors"
                                >
                                  <Eye className="h-4 w-4" />
                                  Watch Quest
                                </button>
            ...
            {activeTab === 'problems' && (
              <motion.div
                ...
                transition={{ duration: 0.3 }}
              >
                <ProblemsTable battleMode="narrative-mode" />
              </motion.div>
            )}
          </div>
          ...
          <div className="lg:col-span-1">
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-semibold text-white">Tavern Chat</h3>
                ...
                {comments.map((comment) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-orange-400 font-medium text-sm">{comment.username}</span>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-gray-800/50 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm placeholder-gray-400 focus:outline-none focus:border-orange-500"
                  onKeyPress={(e) => e.key === 'Enter' && handleSendComment()}
                />
                <button
                  onClick={handleSendComment}
                  disabled={!newComment.trim()}
                  className="bg-orange-600 hover:bg-orange-700 disabled:opacity-50 disabled:cursor-not-allowed p-2 rounded-lg transition-colors"
                >
              ...
              <div className="mt-4 text-center">
                <p className="text-gray-400 text-xs">
                  Share your coding adventure with fellow travelers! 🗡️✨
                </p>
              </div>
            </motion.div>
          </div>
...
export default NarrativeModePage;
